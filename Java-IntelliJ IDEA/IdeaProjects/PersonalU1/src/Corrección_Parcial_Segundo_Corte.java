...
public class Corrección_Parcial_Segundo_Corte {

    /*
    Sebastian Felipe Moreno Gómez       1920010692
    Julian Andres Naranjo Cespedes      1920010409
    Oscar Andres Quiroga Colmenares     1920010401
     */
    ...
    public static void main (String [] args){
        ...
        System.out.print("Digite fila y columna para revisar permiso de nadar: ");
        int fila3_usuario = teclado.nextInt();
        int columna3_usuario = teclado.nextInt();
        boolean permiso = tiene_permiso_de_nadar(matriz_piscina,fila3_usuario,columna3_usuario);
        System.out.println("El niño tiene permiso de nadar: " + permiso);
    }
    ...
    static boolean uno_adyacente(int [][]matriz,int f, int c){
        ...
        if( f==0 && (c!=0 && c!=matriz[0].length-1)){
            if(matriz[f][c-1]==1 || matriz[f+1][c-1] == 1 || matriz[f+1][c] == 1 || matriz[f+1][c+1]
                    == 1 || matriz[f][c+1] == 1){
                return true;
            }
            else{
        ...
        if( f==matriz.length-1 && (c!=0 && c!=matriz[0].length-1)){
            if(matriz[f][c-1]==1 || matriz[f-1][c-1] == 1 || matriz[f-1][c] == 1 || matriz[f-1][c+1]
                    == 1 || matriz[f][c+1] == 1){
                return true;
            }
    ...
    static boolean tiene_permiso_de_nadar(int[][] matriz, int f, int c) {
        if ((borde(matriz, f, c)==true) || (uno_adyacente(matriz, f, c)==true) || matriz[f][c] == 1) {
            return true;
        } else {
            return false;
        }
